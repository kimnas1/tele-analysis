...
def run_vote3_rca_ood(
    *,
    ...
    mod = import_module_from_path("vote3_traces", os.path.join("tools", "generate_submission_rcaood_vote3_traces.py"))

    cfg: Dict[str, Any] = dict(mod.CONFIG)  # type: ignore[attr-defined]
    cfg["RUN_MODE"] = "RCA_OOD"
    ...
    # Overlays from pipeline config (keep logic, only config values).
    ...
    log(f"RCA_OOD vote3: running on {questions_csv}")
